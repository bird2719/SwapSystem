...
class SelectItemWindow extends CustomForm{
    ...
    public function handleResponse(Player $player, $data): void{
        ...
        if(0 < count($items)){
            ...
            $request->setOwner($player->getName());
            $window = new SelectAmountWindow($request, $this->plugin);
            $player->sendForm($window);
        }else{
            $message = "\n" . TextFormat::RED . "交換候補アイテムは１種以上選ぶ必要があります。\n\n";
            $nextWindow = new SelectItemWindow($player, $this->plugin);
